...
public class CurrencyUtils {
	public static String getAllCurrency(EntityPlayer player) {
		...
		for (int i = 0; i < player.inventory.getSizeInventory(); i++) {
			if (player.inventory.getStackInSlot(i) != ItemStack.EMPTY
					&& player.inventory.getStackInSlot(i).getItem() instanceof ItemMoneyBase) {
				ItemMoneyBase money = (ItemMoneyBase) player.inventory.getStackInSlot(i)
						.getItem();
	...
	public static void dropMoneyAmount(float value, World world, double x,
			double y, double z) {
		float newValue = value;

		if (value >= 100) {
			EntityItem hundreds = new EntityItem(world, x, y, z, new ItemStack(
					ItemHandler.hundredDollarBill,
		...
		if (newValue >= 50) {
			EntityItem fifties = new EntityItem(world, x, y, z, new ItemStack(
					ItemHandler.fiftyDollarBill,
					(int) Math.floor(newValue / 50)));
			world.spawnEntity(fifties);
			newValue -= (50 * Math.floor(newValue / 50));
		}

		if (newValue >= 20) {
			EntityItem twenties = new EntityItem(world, x, y, z, new ItemStack(
					ItemHandler.twentyDollarBill,
					(int) Math.floor(newValue / 20)));
			world.spawnEntity(twenties);
			newValue -= (20 * Math.floor(newValue / 20));
		}

		if (newValue >= 10) {
			EntityItem tens = new EntityItem(world, x, y, z, new ItemStack(
					ItemHandler.tenDollarBill, (int) Math.floor(newValue / 10)));
			world.spawnEntity(tens);
			newValue -= (10 * Math.floor(newValue / 10));
		}

		if (newValue >= 5) {
			EntityItem fives = new EntityItem(world, x, y, z, new ItemStack(
					ItemHandler.fiveDollarBill, (int) Math.floor(newValue / 5)));
			world.spawnEntity(fives);
			newValue -= (5 * Math.floor(newValue / 5));
		}

		if (newValue >= 1) {
			EntityItem dollars = new EntityItem(world, x, y, z, new ItemStack(
					ItemHandler.dollarBill, (int) Math.floor(newValue)));
			world.spawnEntity(dollars);
			newValue -= (1F * Math.floor(newValue));
		}

		EntityItem quarters = new EntityItem(world, x, y, z, new ItemStack(
				ItemHandler.quarter, (int) (newValue / 0.25F)));
		world.spawnEntity(quarters);
		newValue -= (0.25F * (int) (newValue / 0.25F));

		EntityItem dimes = new EntityItem(world, x, y, z, new ItemStack(
				ItemHandler.dime, (int) (newValue / 0.10F)));
		world.spawnEntity(dimes);
		newValue -= (0.10F * (int) (newValue / 0.10F));

		EntityItem nickels = new EntityItem(world, x, y, z, new ItemStack(
				ItemHandler.nickel, (int) (newValue / 0.05F)));
		world.spawnEntity(nickels);
		newValue -= (0.05F * (int) (newValue / 0.05F));

		EntityItem pennies = new EntityItem(world, x, y, z, new ItemStack(
				ItemHandler.penny, (int) (newValue / 0.01F)));
