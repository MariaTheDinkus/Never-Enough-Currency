...
public class CurrencyWailaProvider implements IWailaDataProvider {
	...
	public static void callbackRegister(IWailaRegistrar registrar) {
		...
		registrar.registerBodyProvider(dataProvider,
				TileEntityShopController.class);
		registrar.registerNBTProvider(dataProvider,
				TileEntityShopController.class);
		registrar.registerBodyProvider(dataProvider, TileEntityDisplay.class);
		registrar
				.registerBodyProvider(dataProvider, TileEntityStockCrate.class);
	}
	...
	public List<String> getWailaBody(ItemStack itemStack,
		...
		if (accessor.getBlock() instanceof BlockATM) {
			AccountCapability capability = accessor.getPlayer().getCapability(
					Currency.ACCOUNT_DATA, null);
			currenttip.add(I18n.format("currency.account.waila") + " "
					+ capability.getFormattedAmount());
		}

		if (tile instanceof TileEntityShopController) {
			TileEntityShopController controller = (TileEntityShopController) tile;
			NBTTagCompound compound = accessor.getNBTData().getCompoundTag(
					"controller");

			if (controller.getOwnerUUID() != null) {
				currenttip.add(I18n.format("currency.ownername.waila") + " "
						+ controller.getOwnerName());
				currenttip.add(I18n.format("currency.name.waila") + " "
						+ controller.getName());
			}
			...
			if (shelf.getOwnerUUID() != null) {
				currenttip.add("Shelf "
						+ I18n.format("currency.ownername.waila") + " "
						+ shelf.getOwnerName());
				currenttip.add(I18n.format("currency.linked.waila") + " "
						+ shelf.getShopControllerPos().getX() + ", "
						+ shelf.getShopControllerPos().getY() + ", "
						+ shelf.getShopControllerPos().getZ());
			}
			...
			if (crate.getOwnerUUID() != null) {
				currenttip.add("Stock Crate "
						+ I18n.format("currency.ownername.waila") + " "
						+ crate.getOwnerName());
				currenttip.add(I18n.format("currency.linked.waila") + " "
						+ crate.getShopControllerPos().getX() + ", "
						+ crate.getShopControllerPos().getY() + ", "
						+ crate.getShopControllerPos().getZ());
			}
		}
		return currenttip;
	}
	...
	public NBTTagCompound getNBTData(EntityPlayerMP player, TileEntity te,
		...
		if (te instanceof TileEntityShopController) {
			tag.setTag("controller", ((TileEntityShopController) te)
